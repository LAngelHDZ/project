    ...
    @else
        @foreach($temas as $item)
            <div class="flex justify-start content-center">
                @if($item->tipo == 1 )
                    <div class="mt-5">
                        <h4 class=" ">{{ $item->indice.' .- '.$item->nombreTema  }} </h4>
                    </div>
                    <div class="-mt-1">
                        @if($item->cantAct == 1 )
                        ...
                        @else
                        <a href="{{ route('docenteaddActividad',['curso'=> $idCurso,'idtype' => $item->idTema, 'type'=>0]) }}" class="ml-4" title="Agregar Actividad">
                ...
                @elseif($item->tipo == 2 )
                    <div class="flex justify-start content-center">
                        ...
                        <div class="-mt-5">
                            ...
                            @else
                            <a href="{{ route('docenteaddActividad',['curso'=> $idCurso,'idtype' => $item->idTema, 'type'=>0]) }}" class="ml-2 text-xs text-gray-400 hover:text-gray-600" title="Agregar Actividad">
